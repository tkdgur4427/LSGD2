...
static const char *assimp_ShaderCodeVertex = R"(
#version 460 core
...
static const char *assimp_ShaderCodeFragment = R"(
#version 460 core
...
struct Assimp_PerFrameData
{
    mat4 mvp;
    int isWireframe;
};

int Chapter2::Assimp_main()
{
    ...
    glShaderSource(shaderVertex, 1, &assimp_ShaderCodeVertex, nullptr);
    glCompileShader(shaderVertex);

    const GLuint shaderFragment = glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(shaderFragment, 1, &assimp_ShaderCodeFragment, nullptr);
    ...
    const GLsizeiptr kBufferSize = sizeof(Assimp_PerFrameData);
    ...
    // request the library to convery any geometric primitives it might encounter into triangles
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        Assimp_PerFrameData perFrameData = {p * m, (int)false};
        ...
        perFrameData.isWireframe = (int)true;
        glNamedBufferSubData(perFrameDataBuffer, 0, kBufferSize, &perFrameData);
